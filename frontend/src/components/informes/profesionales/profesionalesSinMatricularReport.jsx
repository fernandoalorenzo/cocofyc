...
const ProfesionalesSinMatricularReport = ({ title, nombreInforme }) => {
	...
	useEffect(() => {
		const fetchProfesionalesSinMatricular = async () => {
			try {
        ...
        const response = await apiConnection(endpoint, direction, method, body, headers);

        // Filtrar solo los profesionales Sin Matricular
        const profesionalesSinMatricular = response.data.filter(
            (profesional) => profesional.matricula == ""
        );

        // Ordenar los profesionales matriculados alfabéticamente por nombre
        const sortedData = profesionalesSinMatricular.sort((a, b) => {
		...
		fetchProfesionalesSinMatricular();
	}, []);
...
export default ProfesionalesSinMatricularReport;
