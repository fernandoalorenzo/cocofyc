...
import DenunciasArchivosModal from "../denunciasArchivosModal";
...
const DenunciasSeguimientosModal = ({
	...
	const [selectedDenunciaSeguimiento, setSelectedDenunciaSeguimiento] =
		useState(null);
	const [showDenunciasArchivosModal, setShowDenunciasArchivosModal] =
		useState(false);
	const closeDenunciasArchivosModal = () =>
		setShowDenunciasArchivosModal(false);
	...
	useEffect(() => {
		...
		$(document).on(
			...
			function () {
				...
				archivosSeguimiento(seguimientoId);
			}
		);
	}, [seguimientos]);

	const archivosSeguimiento = (data) => {
		setShowDenunciasArchivosModal(true);
	};
	...
	useEffect(() => {
		...
		if (
			dataSeguimiento &&
			dataSeguimiento.fecha_cierre !== null &&
			dataSeguimiento.fecha_cierre !== "0000-00-00"
		) {
	...
	return (
		<>
			...
			<DenunciasArchivosModal
				showModalArchivos={showDenunciasArchivosModal}
				closeModalArchivos={closeDenunciasArchivosModal}
